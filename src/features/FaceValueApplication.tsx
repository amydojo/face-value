...
export function FaceValueApplication() {
  ...
  const renderContent = () => {
    switch (state.stage) {
      ...
      case 'cabinet': return <><Header /><section className={styles.cabinetHome}><div className={styles.directory}><p>ACTIVE SHELF <span>A1</span></p><p>DAY 12 OF 28</p></div><h1>CABINET</h1><div className={styles.cabinetRows}><button type="button"><span>A1</span><strong>ACTIVE OBSERVATION</strong><small>1</small></button><button type="button"><span>S4</span><strong>ESTABLISHED ROUTINE</strong><small>3</small></button><button type="button"><span>C2</span><strong>COOLING SHELF</strong><small>2</small></button><button type="button"><span>P1</span><strong>PAUSED</strong><small>1</small></button><button type="button" onClick={() => dispatch({ type: 'VIEW_ARCHIVE' })}><span>E7</span><strong>STORED EVIDENCE</strong><small>{Math.max(13, state.archive.length)}</small></button></div><div className={styles.activeAperture} aria-label={`${specimen.accession}, ${specimen.product}, evidence settling`}><span aria-hidden /><strong>{specimen.accession}</strong><p>{specimen.product}</p><small>EVIDENCE SETTLING</small></div><button data-stage-focus type="button" className={styles.nextActionCard} onClick={() => dispatch({ type: 'BROWSE_DRAWERS' })}><span>NEXT VALID ACTION</span><strong>Browse indexed drawers</strong><small>Finite selection · no infinite loop</small><b aria-hidden>→</b></button></section>{nav}</>;
      case 'browse': return <><Header /><section className={styles.drawerBrowse}><div className={styles.directory}><p>A1 · ACTIVE OBSERVATION</p><p>CLOSED</p></div><h1 className={styles.srOnly}>Indexed drawers</h1><DrawerCarousel products={PRODUCTS} index={state.selectedDrawerIndex} onPrevious={() => dispatch({ type: 'PREVIOUS_DRAWER' })} onNext={() => dispatch({ type: 'NEXT_DRAWER' })} onOpen={() => dispatch({ type: 'OPEN_DRAWER' })} /><p className={styles.drawerInstruction}>Swipe between drawers. Open to inspect.</p></section>{nav}</>;
      case 'specimen':

...
export function normalizeCameraError(error: unknown): CameraFailureReason {
  const name = error instanceof DOMException || (typeof error === 'object' && error !== null)
...
export async function requestCamera(
  ...
  try {
    return { ok: true, stream: await mediaDevices!.getUserMedia(CAMERA_CONSTRAINTS) };
  } catch (firstError) {
    const firstReason = normalizeCameraError(firstError);
    if (firstReason === 'overconstrained') {
      try {
        return { ok: true, stream: await mediaDevices!.getUserMedia({ video: true, audio: false }) };
      } catch (fallbackError) {
